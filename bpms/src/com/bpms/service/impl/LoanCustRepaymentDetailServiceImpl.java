...
public class LoanCustRepaymentDetailServiceImpl implements
	...
	public void exportLoanCustRepaymentDetail(Map<String, Object> map)
		...
		String loanCity = StringUtils.isNotBlank((String) map.get("loanCity")) ? (String) map
				.get("loanCity") : "全部";
		String contractNo = StringUtils.isNotBlank((String) map
				.get("contractNo")) ? (String) map.get("contractNo") : "全部";
		String loanName = StringUtils.isNotBlank((String) map.get("loanName")) ? (String) map
				.get("loanName") : "全部";
		String contractSignDateS = StringUtils.isNotBlank((String) map
				.get("contractSignDateS")) ? (String) map
				.get("contractSignDateS") : "/-/-/";
		String contractSignDateE = StringUtils.isNotBlank((String) map
				.get("contractSignDateE")) ? (String) map
				.get("contractSignDateE") : "/-/-/";
		String planRepmtDate = map.get("planRepmtDate") != null ? df
				.format((Date) map.get("planRepmtDate")) : df
				.format(getCurrentPeriodDate());
		String isOverdue = StringUtils
				.isNotBlank((String) map.get("isOverdue"))
				&& map.get("isOverdue").equals("1") ? "当期正常还款" : "逾期还款";
		...
		if (Collections.listIsNotEmpty(modelList)) {
			...
			for (Entry<String, List<CustomerRepaymentInfoModel>> entry : entrySet) {
				...
				if (list.size() > 1) {
					sheet.addMergedRegion(new CellRangeAddress(4 + count
							- list.size(), 4 + count - 1, 0, 0));
				}
				sheet.getRow(4 + count - list.size()).getCell(0)
		...
		response.setHeader(
				...
				"attachment;filename="
						+ URLEncoder.encode(
								"客户贷款明细表-" + calendar.get(Calendar.YEAR) + "-"
	...
	private Map<String, List<CustomerRepaymentInfoModel>> handleCustomerRepaymentInfo(
		...
		if (Collections.listIsNotEmpty(modelList)) {
			...
			for (Entry<String, List<CustomerRepaymentInfoModel>> entry : entrySet) {
				...
				for (CustomerRepaymentInfoModel model : modelList) {
					if (entry.getKey().equals(model.getLoanCity())) {
						list.add(model);
					}
				}
				entry.setValue(list);
			...
			if (info.getPlanRepmtDate() != null) {
				...
				if (day > 0) {
					...
					if (contract != null) {
						...
						BigDecimal add = NumberFormatUtil.formatNumber(2, info
