package com.bpms.model;

// Generated 2015-6-17 10:26:05 by Hibernate Tools 4.3.1

import java.util.Date;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.Id;
import javax.persistence.Table;
import javax.persistence.Transient;

import org.hibernate.annotations.GenericGenerator;

/**
 * AccountInfo generated by hbm2java
 */
@Entity
@Table(name = "t_bp_account_info")
public class AccountInfo implements java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private String accountId;
	private String bankName;
	private String accountName;
	private String accountNum;
	private String cusId;
	private String cusType;
	private String creator;
	private Date createDate;
	private String primaryBackup;// 主副银行卡
	/**
	 * 账户介质
	 */
	private String actMedium;
	/**
	 * 账户性质
	 */
	private String actNature;

	private String stateChecked;

	public AccountInfo() {
	}

	public AccountInfo(String accountId) {
		this.accountId = accountId;
	}

	public AccountInfo(String accountId, String bankName, String accountName,
			String accountNum, String cusId, String cusType) {
		this.accountId = accountId;
		this.bankName = bankName;
		this.accountName = accountName;
		this.accountNum = accountNum;
		this.cusId = cusId;
		this.cusType = cusType;
	}

	@Id
	@GenericGenerator(name = "systemUUID", strategy = "uuid")
	@GeneratedValue(generator = "systemUUID")
	@Column(name = "ACCOUNT_ID", insertable = true, updatable = true, nullable = false, length = 40, unique = true)
	public String getAccountId() {
		return this.accountId;
	}

	public void setAccountId(String accountId) {
		this.accountId = accountId;
	}

	@Column(name = "BANK_NAME", length = 120)
	public String getBankName() {
		return this.bankName;
	}

	public void setBankName(String bankName) {
		this.bankName = bankName;
	}

	@Column(name = "ACCOUNT_NAME", length = 32)
	public String getAccountName() {
		return this.accountName;
	}

	public void setAccountName(String accountName) {
		this.accountName = accountName;
	}

	@Column(name = "ACCOUNT_NUM", length = 100)
	public String getAccountNum() {
		return this.accountNum;
	}

	public void setAccountNum(String accountNum) {
		this.accountNum = accountNum;
	}

	@Column(name = "CUS_ID", length = 40)
	public String getCusId() {
		return this.cusId;
	}

	public void setCusId(String cusId) {
		this.cusId = cusId;
	}

	@Column(name = "CUS_TYPE", length = 30)
	public String getCusType() {
		return this.cusType;
	}

	public void setCusType(String cusType) {
		this.cusType = cusType;
	}

	@Transient
	public String getStateChecked() {
		return stateChecked;
	}

	public void setStateChecked(String stateChecked) {
		this.stateChecked = stateChecked;
	}

	@Column(name = "ACT_MEDIUM", length = 1)
	public String getActMedium() {
		return actMedium;
	}

	public void setActMedium(String actMedium) {
		this.actMedium = actMedium;
	}

	@Column(name = "ACT_NATURE", length = 1)
	public String getActNature() {
		return actNature;
	}

	public void setActNature(String actNature) {
		this.actNature = actNature;
	}

	@Column(name = "CREATOR", length = 40)
	public String getCreator() {
		return creator;
	}

	public void setCreator(String creator) {
		this.creator = creator;
	}

	@Column(name = "CREATE_DATE")
	public Date getCreateDate() {
		return createDate;
	}

	public void setCreateDate(Date createDate) {
		this.createDate = createDate;
	}

	@Column(name = "PRIMARY_BACKUP", length = 1)
	public String getPrimaryBackup() {
		return primaryBackup;
	}

	public void setPrimaryBackup(String primaryBackup) {
		this.primaryBackup = primaryBackup;
	}

}
