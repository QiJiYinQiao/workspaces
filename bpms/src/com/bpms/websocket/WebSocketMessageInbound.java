...
public class WebSocketMessageInbound extends MessageInbound {
	...
	protected void onOpen(WsOutbound outbound) {
		// 触发连接事件，在连接池中添加连接
		/*
		 * JSONObject result = new JSONObject(); result.element("type",
		 * "user_join"); result.element("user", this.user);
		 */
		// 向所有在线用户推送当前用户上线的消息
		/*
		 * WebSocketMessageInboundPool.sendMessage(result.toString());
		 * WebSocketMessageInboundPool.sendMessage("你好呀???");
		 */

		/*
		 * result = new JSONObject(); result.element("type", "get_online_user");
		 * result.element("list", WebSocketMessageInboundPool.getOnlineUser());
		 */
		// 向连接池添加当前的连接对象
		WebSocketMessageInboundPool.addMessageInbound(this);
		// 向当前连接发送当前在线用户的列表
		// WebSocketMessageInboundPool.sendMessageToUser(this.user,
		// result.toString());
	}
