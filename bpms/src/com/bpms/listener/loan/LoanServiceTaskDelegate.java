...
public class LoanServiceTaskDelegate implements Serializable {
	...
	public void ipcDistrbuteByLimit(Execution execution) {
		...
		CreditAuditReport report = creditAuditReportService
				.findCreditAuditReportByLoanOrderId(id);
		// 判断订单的所属机构是否为北京，如果是北京地区则额度为15万，其他地区则为10万。
		int compare = -1;
		// 北京额度为15万
		if (loanOrder.getOrganizationId() == Integer.valueOf(Constants.LOAN_ORGANIZATION_BJ)) {
			compare = report.getActualNeedAmount().compareTo(
