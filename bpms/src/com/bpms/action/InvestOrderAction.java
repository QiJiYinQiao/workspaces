...
public class InvestOrderAction extends BaseAction implements ModelDriven<InvestOrder> {
	...
	public String checkWorkFlowImg(){
		try {
			investOrderWorkFlowService.checkWorkFlowImgByInvestOrderId(investOrder.getInvestOrderId());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public String findListByInvestorAndInvestOrder(){
	    ...
	    gridModel.setRows(investOrderServiceImpl.findListByInvestorAndInvestOrder(pageUtil));
	...
	public String doDeleteInvestOrder(){
		boolean bl = investOrderServiceImpl.doDeleteInvestOrder(investOrder.getInvestOrderId());
		DataModel json = null;
		if(bl){
	...
	public String findAllTaskList(){
		GridModel gridModel = new GridModel();
		gridModel.setRows(investOrderWorkFlowService.findAllTask(getFirstResult(), getMaxResults()));
		gridModel.setTotal(investOrderWorkFlowService.findAllTaskCount());
		OutputJson(gridModel);
		return null;
	}
	/**
	...
	public String findTaskFormKeyByTaskId(){
		OutputJson(workFlowService.findTaskFormKeyByTaskId(investOrder.getTaskId()));
		return null;
	}	
...
//				searchColumnNames, searchConditions, searchVals);		
		...
		List<InvestOrder> rows = investOrderWorkFlowService.findAllClaimTask(getFirstResult(), getMaxResults());

		//获取当前"角色（比如，销客专员）"已经受理的所有投资订单investOrder的"总数量"
		Long total = investOrderWorkFlowService.findAllClaimTaskCount();
	...
	public String findInvestorOrderContractDetails(){
		...
		InvestOrder investOrderObj = investOrderServiceImpl.findByInvestOrderId(investOrderId);
		ServletActionContext.getRequest().setAttribute("investOrderObj", investOrderObj);							
		
		//查询“理财产品”的数据。				
		List<InvestorAndInvestProductModel> investProductList = investOrderServiceImpl.findInvestorAndInvestProductsDetails(investOrderId);
		ServletActionContext.getRequest().setAttribute("investProductList", investProductList);		
	...
	public String findInvestorAndInvestProductsDetails(){		
		List<InvestorAndInvestProductModel> InvestorAndInvestProductRows = investOrderServiceImpl.findInvestorAndInvestProductsDetails(investOrder.getInvestOrderId());
		ServletActionContext.getRequest().setAttribute("InvestorAndInvestProductRows", InvestorAndInvestProductRows);					
	...
	public String findByInvestOrderId(){
		OutputJson2(investOrderServiceImpl.findByInvestOrderId(investOrder.getInvestOrderId()));
		return null;
	}
	/**
	 * 提交任务办理
	...
	public String submitTask() {
		try {
			investOrderWorkFlowService.saveSubmitTask(investOrder, comment, result,
					processingResult);
	...
	public String findInvestOrderListByOrderStatus(){
		GridModel gridModel = new GridModel();
		gridModel.setRows(investOrderServiceImpl.findInvestOrderListByOrderStatus());
		OutputJson(gridModel);
		return null;
	}
	/**
	...
	public String findInvestOrderListByInvestOrderIds(){
		GridModel gridModel = new GridModel();
		gridModel.setRows(investOrderServiceImpl.findInvestOrderListByInvestOrderIds(investOrderIds));
		OutputJson(gridModel);
		return null;
	}
	@Override
