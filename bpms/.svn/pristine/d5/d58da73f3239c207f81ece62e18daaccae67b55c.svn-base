...
$(function(){
	...
	function  submitTask(result,object) {
		var choose = $("#processingResult").combobox("getValue");
		if("B"==choose) {
			// 验证备注信息是否已经填写
		...
		$.messager.confirm('提示', '点击按钮之后将进入下一个任务活动环节,此任务将对您不可见!', function(r){
			if (r){
				var data = {
						...
						"auxiliary":$("#auxiliary").combobox("getValue")
				}
				$.ajax({
					type : "POST",
	...
	function ipcMicrocreditOpinionDailog(){
		window.open("jsp/loanOrder/IPC/ipcMicrocreditOpinion.jsp?loanOrderId="+$row.loanOrderId+"&loanerName="+$row.name+"&loanerIdNo="+$row.idNo+"&purpose="+$row.purpose,
				"呈报意见详情", "height="+($(window).height()*0.8)+", width=900, top=100, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");
	}
...
<div data-options="region:'north',title:'North Title',split:true">
	<div style="width: 986px;height: 200px;overflow: auto;">
			<form id="acceptTaskForm" method="post">
				 ...
				 <table class="table" cellpadding="5px;">
					...
					<tr>
						...
						<th>是否补调:</th>
						<td><input id="auxiliary" name="auxiliary" class="easyui-validatebox easyui-combobox" type="text"/></td>
					</tr>
					<tr>
					 	<th>备注简述:</th>
						<td>
		...
		<div style="width: 900px;height:30px;">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCLRCThrough');">提交</a>
		</div>
		
		<div id="lookInfo" class="easyui-accordion" style="height: 365px;width: 900px;overflow: hidden;">
