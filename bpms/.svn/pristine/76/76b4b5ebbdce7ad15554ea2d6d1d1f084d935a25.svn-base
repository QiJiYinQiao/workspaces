...
<script type="text/javascript">
	...
	function  submitTask(result) {
		// 验证备注信息是否已经填写,如果拒绝才验证
		if(result=="IPCTrialSupervisorReject") {
		...
		$.messager.confirm('提示', '点击按钮之后将进入下一个任务活动环节,此任务将对您不可见!', function(r){
			if (r){
				var data = {
					...
					"taskId" :$row.taskId,
					"processingResult":result=="IPCTrialSupervisorThrough"?"A":"B"
				}
				$.ajax({
					type : "POST",
...
<div data-options="region:'north',title:'North Title',split:true" style="overflow: hidden;">
	...
	<div style="width: 900px;height:30px;">
		...
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCTrialSupervisorThrough');" >电核部主管通过</a>
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCTrialSupervisorReject');" >电核部主管拒贷</a>	
	</div>
