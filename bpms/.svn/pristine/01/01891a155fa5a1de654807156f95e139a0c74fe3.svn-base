...
$(function(){
		...
		$.messager.confirm('提示', '点击按钮之后将进入下一个任务活动环节,此任务将对您不可见!', function(r){
			if (r){
				var data = {
					...
					"taskId": $row.taskId,
					"processingResult":result=="ApprovalLeaderThrough"?"A":"B"
				}
				$.ajax({
					type : "POST",
	...
	function checkApplicationReportDetail(){
		window.open("jsp/loanOrder/applicationReportDetail.jsp?loanOrderId="+$row.loanOrderId+"&loanerId="+$row.loanerId+"&isFinalShow=1",
				"稽核信息详情", 'height=650, width=1000, top=200, left=400, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')
	}
</script>
<!-- 受理任务 S -->
<div data-options="region:'north',title:'North Title',split:true">
	...
	<div style="width: 980px;height:30px;">
		...
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('ApprovalLeaderThrough',this);">审批经理通过</a>
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('ApprovalLeaderRefuse',this);">审批经理拒贷</a>
	</div>	
		
	<div id="lookInfo" class="easyui-accordion" style="height: 390px;width: 980px;overflow: hidden;">
	    <div title="备注信息" data-options="iconCls:'icon-cstbase',selected:true" style="padding:10px;">   
