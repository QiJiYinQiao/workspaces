...
public class LoanIPCSendEmailExecutionListener implements ExecutionListener {
	...
	public void notify(DelegateExecution execution) throws Exception {
		...
		List<HistoricTaskInstance> hisTaskList = historyService.createHistoricTaskInstanceQuery().processInstanceId(pid).taskDefinitionKey((String)getDefId().getValue(execution)).orderByTaskCreateTime().desc().list();
		...
		HistoricVariableInstance hVariable2 = historyService.createHistoricVariableInstanceQuery().processInstanceId(pid).taskId(hisTaskList.get(0).getId()).variableName("role").singleResult();
		...
		if(Collections.listIsNotEmpty(users)){
			for (Users user : users) {
				MimeMessage message = SendEmailUtil.createTextMail("贷款申请已经被拒贷!", "订单号:"+order.getLoanerId()+"\n客户名:"+loaner.getName()+"\n已经被"+role.getName()+":"+Constants.getCurrendUser().getUser().getName()+"拒贷!!!", user.getEmail());
