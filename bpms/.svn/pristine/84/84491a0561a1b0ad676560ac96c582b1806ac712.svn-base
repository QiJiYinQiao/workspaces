...
public class ContactsServiceImpl implements ContactsService {
	...
	public List<ContactsModel> findContactsByOid(String oid) {
		...
		if(Collections.listIsNotEmpty(cts)){
			for(Contacts c : cts){
				String sql = "SELECT t.DICT_NAME from t_sys_dict t where t.PARENT_ID = (select td.CODE_ID from t_sys_dict td where td.DICT_CODE = 'relationship_type') AND t.DICT_CODE = '"+c.getRelationship()+"'";
