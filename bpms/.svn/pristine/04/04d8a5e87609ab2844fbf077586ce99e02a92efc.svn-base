...
public class MyShiroRealm extends AuthorizingRealm {
		...
		if (username != null) {
			...
			if (Constants.SYSTEM_ADMINISTRATOR.equals(username)) {
				sql = "SELECT p.PERMISSION_ID,p.MYID FROM T_PERMISSION AS p\n"
						+ "where p.STATUS='A' and p.TYPE='O' and p.ISUSED='Y' and p.SYSTEM_TYPE='1'";
			} else {
	...
	private List<String> findRoleCodesByUserId(Integer userId) {
		...
		sb.append("LEFT JOIN t_user_and_role tur ON tu.USER_ID = tur.USER_ID ");
		sb.append("LEFT JOIN t_role tr ON tr.ROLE_ID = tur.ROLE_ID ");
		sb.append("WHERE ");
