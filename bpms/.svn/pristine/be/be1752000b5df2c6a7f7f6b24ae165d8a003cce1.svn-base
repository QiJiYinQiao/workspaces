...
public class LoanIPCPersonInChargeSendEmailExecutionListener implements ExecutionListener {
	...
	public void notify(DelegateExecution execution) throws Exception {
		...
		List<HistoricTaskInstance> hisTaskList = historyService.createHistoricTaskInstanceQuery().processInstanceId(pid).taskDefinitionKey("IPCData").orderByTaskCreateTime().desc().list();
		List<HistoricTaskInstance> hisTaskList2 = historyService.createHistoricTaskInstanceQuery().processInstanceId(pid).taskDefinitionKey((String)getDefId().getValue(execution)).orderByTaskCreateTime().desc().list();
		...
		HistoricVariableInstance hVariable2 = historyService.createHistoricVariableInstanceQuery().processInstanceId(pid).taskId(hisTaskList2.get(0).getId()).variableName("role").singleResult();
		String ipcDataUser = hisTaskList.get(0).getAssignee();
