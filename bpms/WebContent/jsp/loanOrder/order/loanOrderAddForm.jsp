	   ...
	   renderProvinceSelect("corpProvinceId","corpCityId","corpAreaId");
		...
		$("#tt").tabs({
			onSelect:function(title,index){
				 if($addRow!=null){
					if(1==index){
						linkPeopleDatagrid({"loanerId": $addRow.loanerId,"loanOrderId":$("#loanOrderId").val()});//紧急联系人列表
					}else if(2==index){
						initDatagrid({"loanerId": $addRow.loanerId,"loanOrderId":$("#loanOrderId").val()}); //工作单位列表
					}else if(4==index){
						initBankGrid({"loanerId": $addRow.loanerId,"loanOrderId":$("#loanOrderId").val()});//开户行列表
					}
				  }
			   }
		 });
				   ...
				   <form id="baseForm" method="post">
						...
						<table class="table">
							<tr>
								...
								<th>性别:</th>
								<td><input id="baseGenderType" name="genderType" /></td>
							</tr>
							<tr>
								<th>手机:</th>
							...
							<tr>
								...
								<th>婚姻状况:</th>
								<td><input id="baseMarriageType" name="marriageType"/></td>
								<th>有无子女:</th>
								<td><input id="baseHasChild" name="hasChild" /></td>
							</tr>
							<tr>
								<th>年收入(元):</th>
								...
								<th>居住情况:</th>
								<td><input id="baseMortgageStatus" name="mortgageStatus"/></td>
							</tr>
							<tr>
							    <th>月供(元):</th>
							    <td>
