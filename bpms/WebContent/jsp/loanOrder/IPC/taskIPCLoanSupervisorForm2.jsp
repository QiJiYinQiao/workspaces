...
<script type="text/javascript">
	...
	$(function(){
		...
		//渲染电荷专员的信息
		$('#assessorId').combogrid({    
		    panelWidth:450,    
		    idField:'userId',    
		    textField:'name',  
			queryParams: {"roleCode":"IPCShenHeZhuanYuan"},
	...
	function  submitTask(result) {
		// 判断是否含有电核专员信息
		if($("#assessorId").combogrid("getValue")==""){
			$.messager.alert("提示","请指定审核专员!","warning")
			return false;	
		}
		// 验证备注信息是否已经填写
		...
		$.messager.confirm('提示', '点击按钮之后将进入下一个任务活动环节,此任务将对您不可见!', function(r){
			if (r){
				var data = {
					...
					"taskId" :$row.taskId,
					"processingResult":result=="IPCLoanSupervisor2Through"?"A":"B",
					"assessorId":$("#assessorId").combogrid("getValue")
...
<div data-options="region:'north',title:'North Title',split:true" style="overflow: hidden;">
		...
		<form id="acceptTaskForm" method="post">
				...
				<input name="auditId" type="hidden" value="noauditId"/>
				 <table class="table" cellpadding="4">
					...
					<tr>
						<th>审核专员:</th>
						<td><input id="assessorId" name="assessorId " readonly="readonly" type="text"/></td>
					 	<th>备注简述:</th>
						<td>
	...
	<div style="width: 900px;height:30px;">
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCLoanSupervisor2Through');" >审贷部主管指派完毕</a>
	</div>
