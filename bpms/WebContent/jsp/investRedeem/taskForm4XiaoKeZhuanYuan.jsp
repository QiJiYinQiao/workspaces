...
<script type="text/javascript">
	...
	$(function(){
		...
		//该函数的作用：
		//1.获得输入的回款日期，并且对 回款日期的格式和类型进行转换
		//2.判断输入的回款日期的正确性：回款日期  > 赎回申请日期
		//3.计算实际理财天数：回款日期  - 赎回申请日期
		function onChange4HuiKuanDate(){
			...
			//2.判断输入的回款日期的正确性：回款日期要大于赎回申请日期	 		
			//(1) 赎回申请日期的格式和类型转换
			var redeemBeginDate = row.redeemBeginDate;
			strRedeemBeginDate = redeemBeginDate.replace(/-/g, '/'); // "2010/08/01";
			var dateRedeemBeginDate = new Date(strRedeemBeginDate);
			
			//(2) 判断输入的回款日期的正确性：回款日期要大于赎回申请日期
			var daysDiff = dateHuiKuanDate.diff(dateRedeemBeginDate);
			if(daysDiff < 0 ){
	 			$.messager.alert("提示","回款日期要大于赎回申请日期"+strRedeemBeginDate+"，请选择回款日期后再进行提交!","warning");
	 			$("#huiKuanDate").datebox("clear");
	 			return false;								
			}else{
				...
				//(1) 计息日期的格式和类型转换
		 		...
		 		//(2)实际投资天数（回款日期 - 计息日期）
		 		actualInvestDays = dateHuiKuanDate.diff(dateInterestDate) + 1;  // 注意：还要加上“计息日期”这一天。		 		
		 		console.info("计息日期："+dateInterestDate+"________回款日期："+dateHuiKuanDate+"————————实际理财天数（daysDiff）："+ actualInvestDays );	 		
...
<!-- 	<div style="height: 280px;overflow: auto;" > -->
	<div style="margin-left: 160px;">
		<form id="acceptTaskForm" method="post">
			 ...
			 <input name="redeemBeginDate" type="hidden"/>			
