	...
	jqueryUtil.chgSkin=function(selectId,cookiesColor){
      ...
      //高级查询
		...
		jqueryUtil.getTextArr = function(type){
			...
			$.ajax({
				type:'POST',
				url:'common/commonAction!findTextArr.action',
				data:"codeMyid="+type,
				async:false,
				dataType:'json',
		...
		jqueryUtil.getAreaTextArr = function(mainID){
			...
			$.ajax({
				type:'POST',
				url:'common/commonAction!getAreaDicText.action',
		...
		//回显类型
		jqueryUtil.showText = function(value,err){
			var text = "";
			if(null==value||null==err){
				return text;
			}
			for (var i in err) {
				if(err[i].code == value){
		 ...
		 * value 值
		 */
		jqueryUtil.showDicText = function(code,value){
			//查询字典，放回数组
			var arr = new Array();
			$.ajax({
				type:'POST',
