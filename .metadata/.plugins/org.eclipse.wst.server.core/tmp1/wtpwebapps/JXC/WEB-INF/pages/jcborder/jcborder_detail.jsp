...
<body class="easyui-layout" >
	<div class="indiv" style="height:100%;width:100%;background-color: #F8F8FF;overflow: hidden;" >
		...
		<tr>
			...
			<td>
				<%-- <select id="isphoneconfirm" style="border: 1px solid #95b8e7;border-radius: 5px;height: 24px;width: 140px;" name="isphoneconfirm">
					<option value="1" <c:if test="${jcborder.isphoneconfirm eq 1}">selected="selected"</c:if>>是</option>
					<option value="0" <c:if test="${jcborder.isphoneconfirm eq 0}">selected="selected"</c:if>>否</option>
				</select> --%>
				<input  type="text" name="upassport" value="${jcborder.isphoneconfirm }" readonly="readonly" ></input></td>
			</td>
		...
		<tr>
			<td >电话备注</td>
			<td colspan="7">
			<textarea  style="border: 1px solid #95b8e7;border-radius: 5px;height: 60px;width:90%;resize:none;overflow-Y:scroll;font-size: 18px;background-color:rgb(240,248,255);" resize="none" id="phoneremark" readonly="readonly" name="phoneremark">${jcborder.phoneremark }</textarea>
			</td>
		</tr>
		</table>	
	</div>
