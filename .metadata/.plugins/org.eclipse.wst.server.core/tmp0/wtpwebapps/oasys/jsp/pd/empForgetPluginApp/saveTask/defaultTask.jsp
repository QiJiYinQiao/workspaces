...
function toSaveOrUpdateInvestProductOpenDlg(){
	...
	$tiaoZhengWindow = $("<div></div>").dialog({
		...
		buttons : [ {
	    	text:'保存',
	    	iconCls : 'icon-save',
			handler:saveEditEmpForgetPluginApp
	    },{
			text : '关闭',
			iconCls : 'icon-cancel',
			handler : function() {
...
function saveTaskFunc(isSuccess){
	...
	$.ajax({
		type: "POST",
		url:"empForgetPluginAppController/submitTask.do",
		data:data1,
	    success: function(msg) {
 	    	if(msg.status){
    ...
    <form id="taskForm"  method="post" style="width: 98%">
        <input id="applyStr" name="applyStr" type="hidden"  value="<%=Constants.APPLY_FOR_ADJUSTMENT %>" />
 		<table class="table">
				...
				<tr>
					<th align="right">忘打卡事由:</th>
					<td colspan="5">
						<textarea name="forgetPluginReason" class="easyui-textbox" style="width: 99%; height: 90px;" disabled="disabled"></textarea>
					</td>
				</tr>
				<tr>
