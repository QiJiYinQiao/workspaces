...
<script type="text/javascript">	
	...
	function saveApp(){
		if($("input[type='checkbox']").is(":checked")){
			...
			if($("#planUseDate").datebox("getText")!=null && $("#planRestDate").datebox("getText")!=null && $("#planUseDate").datebox("getText")!="" && $("#planRestDate").datebox("getText")!=""){
				$("#selectedCheckbox").val(selectedCheckbox);
				$('#investProductInputOrSaveForm').submit();
			}else{
				$.messager.alert("提示","请选择计划使用时间和计划规划时间!","warning");
			}
		}else{
			$.messager.alert("提示","请选择要申请的证章!","warning");
		}
	};
	//展示附表数据
	$(function(){
		...
		$('#investProductInputOrSaveForm').form({    
		    ...
		    success:function(data){
				...
				$("#travelExpensesAppgrid").datagrid("reload");//主页面
// 				$("#saveDialog").dialog('close');考虑有多条证章，所以修改后不自动关闭
				$("#credentialsAttachDatagrid").datagrid("reload");//编辑时用到,附件表
		    }    
		});
		if($row!=null){
			if($row.appNo!=null && $row.appNo!=""){
				$("#credentialsAttachDatagrid").datagrid({
					url : 'credentials/getAttachList.do?appNo='+$row.appNo,
					width: 'auto',
					height : 200,
					pagination:true,
					rownumbers:true,
			...
			<table class="table" width="100%">
				<tr>
					...
					<td>
						...
						<a style="margin-left: 20px;">使用原因:</a><input name="useDescCaiWuZhang" id="useDescCaiWuZhang" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
					</td>
				</tr>
				<tr>
					<th></th>
					<td>
						...
						<a style="margin-left: 20px;">使用原因:</a><input name="useDescFaRenZhang" id="useDescFaRenZhang" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
					</td>
				</tr>
				<tr>
					<th>证件资质:</th>
					<td>
						...
						<a style="margin-left: 20px;">使用原因:</a><input name="useDescFaRenShenFenZheng" id="useDescFaRenShenFenZheng" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
					</td>
				</tr>
				<tr>
					<th></th>
					<td>									
						...
						<input name="HeTongName" id="HeTongName" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
						...
						<a style="margin-left: 20px;">使用原因:</a><input name="useDescHeTong" id="useDescHeTong" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
					</td>
				</tr>
				<tr>
					<th></th>
					<td>
						...
						<input name="ZiZhiName" id="ZiZhiName" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
						...
						<a style="margin-left: 20px;">使用原因:</a><input name="useDescZiZhi" id="useDescZiZhi" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
					</td>
				</tr>
				<tr>
					<th></th>
					<td>
						...
						<input name="QiTaName" id="QiTaName" class="easyui-textbox" data-options="validType:'length[0,100]', required:true" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
						...
						<a style="margin-left: 20px;">使用原因:</a><input name="useDescQiTa" id="useDescQiTa" class="easyui-textbox" data-options="validType:'length[0,256]', required:true" maxlength="256"/>
					</td>
				</tr>
			    <tr>
					<th>计划使用日期:</th>
					<td>
						<input name="planUseDate" id="planUseDate" class="easyui-datebox" editable="false"/>
					</td>
				</tr>
				<tr>
					<th>计划归还日期:</th>
					<td>
						<input name="planRestDate" id="planRestDate" class="easyui-datebox" editable="false"/>
					</td>
				</tr>
				<tr>
					<th>备注:</th>
