		...
		<table class="table"  width="95%">
			...
			<tr>
			   ...
			   <td colspan="5" align="left">
			      	<textarea name="remark" id="remark" class="easyui-textbox easyui-validatebox resize" data-options="validType:'length[0,100]'" style="width: 600px; height: 90px;" value=value="${consumablesApp.remark }">${consumablesApp.remark }</textarea>
			   </td>
			</tr>	
			<tr>
				<td colspan="3"></td>
...
function createConsumablesAppAttachGrid(){
	$("#consumablesAppAttachGrid").datagrid({
		...
		fitColumns:true,
		columns:[[
			{field : 'caaId',        title : 'CAA_ID',    width : 150, align:'center' },
			{field : 'goodsName',        title : '用品名称',    width : 150, align:'center' },
			{field : 'model',      title : '规格型号', width : 150, align:'center'},
			...
			{field : 'qty',    title : '数量',    width : 150, align:'center' },
			{field : 'totalAmtStr',    title : '合计金额',    width : 150, align:'center' ,
	        	 formatter: function(value,row,index){
	        		if(row.totalAmt.toString().indexOf(".")<0){
						 return row.totalAmt.toString().replace(/(\d)(?=(\d{3})+$)/g,"$1,")+".00"
					}else{
						 return row.totalAmt.toString().split(".")[0].replace(/(\d)(?=(\d{3})+$)/g,"$1,")+"."+row.totalAmt.toString().split(".")[1];
      				}
	        	}
			}
	   ]],
