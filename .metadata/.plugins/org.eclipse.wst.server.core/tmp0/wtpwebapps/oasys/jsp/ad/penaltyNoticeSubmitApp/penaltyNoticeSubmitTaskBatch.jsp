...
<html>
  <head>
	...
	<script type="text/javascript">
			...
			function signTask(){
				...
				for(var i=0;i<rows.length;i++){
					if($.inArray(rows[i].penaltyNoticeSubmitApp.pnrId,businessID)==-1){
						businessID.push(rows[i].penaltyNoticeSubmitApp.pnrId);
						formKey=rows[i].penaltyNoticeSubmitApp.formKey;
						taskID.push(rows[i].penaltyNoticeSubmitApp.taskID);
						appNo.push(rows[i].penaltyNoticeSubmitApp.appNo);
					}
				}
				var data1 = "appNo="+appNo.join(",")+"&businessID="+businessID.join(",")+"&taskID="+taskID.join(",")+"&formKey="+formKey;
			 	  $.ajax({
					type: "POST",
					url:"penaltyNoticeSubmitAppController/saveTaskPenaltyNoticeSubmitAppBatch.do",
					data:data1,
				    success: function(iJson) {
				    	$("#dg").datagrid("load",{});
				    	ajaxLoadEnd();
  ...
  <body>
     ...
     <div class="position" style="margin-top: 5px;">您当前所在位置： 行政办公  &gt; 罚款通知提交申请 </div>
     <div class="well well-small" style="margin-left: 5px;margin-top: 5px">
			<form id="searchForm" action="penaltyNoticeSubmitAppController/findPenaltyNoticeSubmitAppList.do" method="post" style="min-width: 1200px;">
