...
function createMyTaskGrid(){
	$("#taskList").datagrid({
		url:'investApply/investApplyAction!findAllUnclaimedTaskList.action',	//获取当前角色中所有没有被签收（指定）的任务	
		title:'待办任务',
		width: 'auto',
		...
		columns:[[
		        {field : 'investorName',title : '客户姓名', width : parseInt($(this).width() * 0.06),align : 'center',formatter:function(value,row,index){
	            ...
	            {field : 'mobTel',title : '手机号码', width : parseInt($(this).width() * 0.08),align : 'center'},
	            {field : 'licaichanping',title : '理财产品', width : parseInt($(this).width() * 0.06),align : 'center',formatter:function(value,row,index){
               	 var result ="<a href='javascript:void(0);' onclick='showInvestorAndInvestProductsDetailsView(\""+row.investOrderId+"\");'>详情</a>";
    				 return result;	
                }},
                {field : 'financingMgr',title : '理财经理', width : parseInt($(this).width() * 0.06),align : 'center',
                ...
                {field : 'actNo',title : '开户行账号', width : parseInt($(this).width() * 0.06),align : 'center'},
                {field : 'actName',title : '账户名称', width : parseInt($(this).width() * 0.06),align : 'center'},
                {field : 'fmSignDate',title : '理财经理签署日期', width : parseInt($(this).width() * 0.07),align : 'center'},
...
function claimTask(index){
	...
	$.ajax({
		type:'POST',
		url:'investApply/investApplyAction!pickMyTask.action',
...
function checkProcessImg(index){
	var rows = $("#taskList").datagrid("getRows");
	var row = rows[index];
/* 	var src = "investOrder/investOrderAction!checkWorkFlowImg.action?investOrderId=" + row.investOrderId; */
...
function checkInvestOrderOpinions(index){
	...
	$('#OpinionsDialog').dialog({    
	    ...
	    closed: false,    
	    cache: false,    
 	    /* href: 'jsp/investApply/optionsList.jsp', */     
	    href: 'jsp/investApply/optionsList4InvestApplyDaiBan.jsp',     
	    modal: true   
	});   
}
//查看该投资人的理财产品详情界面
