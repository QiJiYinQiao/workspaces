	...
	<script type="text/javascript">
			...
			$(function() {
				...
				$(window).resize(function(){  
				 ...
				 $grid=$dg.datagrid({
					url : "investRedeem/investRedeemAction!findAllClaimedTask.action", 
					width : 'auto',
					...
					columns : [ [ 
					              ...
					              {field : 'operType',       title : '操作',width : parseInt($(this).width()*0.15), align:'center', 
					            	  formatter: function(value,row,index){
		      							var result ="<a href='javascript:void(0);' onclick='checkInvestOrderOpinions("+index+");'>查看审批意见</a>　 ";
		...
		// 根据索引获取行的基本信息的函数
		function getRowDataInShouLiMainJsp (index) {
		...
		function showInvestContractDetailsView(index){
			var row = getRowDataInShouLiMainJsp(index);
		...
		function handleTaskDialog(index) {
			...
			$.ajax({
				type : "POST",
				url : "investRedeem/investRedeemAction!findTaskFormKeyByTaskId.action",
