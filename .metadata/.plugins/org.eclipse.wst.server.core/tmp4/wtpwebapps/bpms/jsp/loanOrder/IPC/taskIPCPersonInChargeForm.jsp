...
$(function(){
		...
		$.messager.confirm('提示', '点击按钮之后将进入下一个任务活动环节,此任务将对您不可见!', function(r){
			if (r){
				var data = {
					...
					"taskId": $row.taskId,
					"processingResult":result=="IPCPersonInChargeReject"?"A":"B"
				}
				$.ajax({
					type : "POST",
	...
	function checkApplicationReportDetail(){
		window.open("jsp/loanOrder/IPC/ipcApplicationReportDetail.jsp?loanOrderId="+$row.loanOrderId+"&loanerId="+$row.loanerId,
				"信审报告详情", "height="+($(window).height()*0.8)+", width=900, top=100, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no")
	}
	
	//查看呈报意见决议表
	function  ipcMicrocreditOpinionRefuseSuggestionInfo(){
		window.open("jsp/loanOrder/IPC/ipcMicrocreditOpinionRefuseSuggestionInfo.jsp?loanOrderId="+$row.loanOrderId+"&loanerName="+$row.name+"&loanerIdNo="+$row.idNo+"&purpose="+$row.purpose+"&loanAmount="+$row.loanAmount,
				"拒绝决议表详情", "height="+($(window).height()*0.8)+", width=900, top=100, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no")
	}
</script>
<!-- 受理任务 S -->
<div data-options="region:'north',title:'North Title',split:true">
	...
	<div style="width:900px;height:30px;">
		...
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="ipcMicrocreditOpinionRefuseSuggestionInfo();">查看调查决议表</a>
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCPersonInChargeReject');">IPC部门主管驳回</a>
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCPersonInChargeRefuse');">IPC部门主管拒贷</a>
	</div>
