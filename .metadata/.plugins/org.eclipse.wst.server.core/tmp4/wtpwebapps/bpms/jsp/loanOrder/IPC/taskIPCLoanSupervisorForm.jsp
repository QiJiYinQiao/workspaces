...
$(function(){
   ...
   //查询终审信贷方式，并加载
   /* $.ajax({
	  url : "finalAuditReportAction/finalAuditReportAction!findFinalAuditReportByLoanOrderId.action",
	  type : "post",
	...
	function  submitTask(result) {
		// 验证备注信息是否已经填写
		if(result!="IPCLoanSupervisorThrough") {
		...
		$.messager.confirm('提示', '点击按钮之后将进入下一个任务活动环节,此任务将对您不可见!', function(r){
			if (r){
				  /*var finaId = $("#finaId").val();
					...
					$.ajax({
						type : "POST",
						url : "finalAuditReportAction/finalAuditReportAction!updateFinalAuditWay.action",
						data : {"finaId":finaId,"auditWay":auditWay},
						success : function(data) {
							var data = {
								"comment" : $("#comment").val(),
								"result" :result,
				...
				var data = {
						...
						"loanOrderId" : $row.loanOrderId,
						"taskId": $row.taskId,
						"processingResult":result=="IPCLoanSupervisorThrough"?"A":"B"
					}
				$.ajax({
					type : "POST",
					url : "loanOrder/loanOrderAction!submitTask.action",
...
<div data-options="region:'north',title:'North Title',split:true">
	...
	<div style="width: 900px;height:30px;">
		...
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="checkOurSurveyReport();">查看小额调查报告及总结</a>
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCLoanSupervisorThrough');">审贷主管通过</a>
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCLoanSupervisorReject');">审贷主管驳回</a>
		<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('IPCLoanSupervisorRefuse');" >审贷主管拒贷</a>	
	</div>
