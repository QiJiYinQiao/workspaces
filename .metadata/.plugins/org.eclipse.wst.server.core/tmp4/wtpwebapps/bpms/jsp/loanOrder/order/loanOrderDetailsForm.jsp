...
<script type="text/javascript" src="jsp/loanOrder/order/loanOrderBaseForm.js"></script>
...
$(function(){
	...
	$("#tt").tabs({
		 onSelect:function(title,index){
			 if(0==index){
				...
				linkPeopleDatagrid({"loanerId": $row.loanerId,"loanOrderId": $row.loanOrderId,"operate":"info"});//紧急联系人列表
			 }else if(2==index){
				initDatagrid({"loanerId": $row.loanerId,"loanOrderId": $row.loanOrderId,"operate":"info"});//工作单位列表
			 }else if(3==index){
				 loadLoanOrderInfo($row);//渲染订单的信息
			 }else if(4==index){
				initBankGrid({"loanerId": $row.loanerId,"loanOrderId": $row.loanOrderId,"operate":"info"});//开户行列表
			 }else if(5==index){
	...
	//渲染紧急联系人列表
	function linkPeopleDatagrid(queryParams) {
		$("#linkPeople").datagrid({
				width : 863,
				height : 290,
				pagination : false,
				...
				nowrap : true,
				multiSort : false,
				url:"contacts/contactsAction!findAllListChacked.action",
				queryParams :queryParams,
				columns : [[
							{field : 'ck',rowspan : 2,checkbox : true},
							{field : 'chName',title : '姓名',width : 80,rowspan : 2,align : 'center'},
							{field : 'relationshipName',title : '与本人关系',width : 80,rowspan : 2,align : 'center'},
						...
						[
							{field : 'compProvinceName',title : '省',width : 80,align : 'center'},
							{field : 'compCityName',title : '市',width : 80,align : 'center'},
							{field : 'compAreaName',title : '县/区',width : 80,align : 'center'},
	...
	function initDatagrid(queryParams) {
		$("#dwDatagrid").datagrid({
				...
				nowrap : true,
				multiSort : false,
				url: "company/companyAction!findAllListChacked.action",
				queryParams:queryParams,
				columns : [[
				            {field : 'ck',rowspan : 2,checkbox : true},
				            {field : 'name',title : '单位名称',width : 80,rowspan : 2,align : 'center'},
				            {field : 'tele',title : '单位电话',width : 100,rowspan : 2,align : 'center'},
				             {field : 'corpNatureName',title : '单位性质',width : 140,rowspan:2,align : 'center'},
						...
						[
							{field : 'dwProvinceName',title : '省',width : 80,align : 'center'},
							{field : 'dwCityName',title : '市',width : 80,align : 'center'},
							{field : 'dwAreaName',title : '县/区',width : 80,align : 'center'},
					...
					<div class="well well-small" style="margin-left: 5px;margin-top: 5px;width:99%;">
					  <form id="loanInfo">
					    <table class="table">
							<tr>
								<th>申请额度(元):</th>
								<td><input name="loanAmount" class="easyui-numberbox" data-options="groupSeparator:','" disabled="disabled"/>
								<th>最低接受额度(元):</th>
								<td><input name="loanMin" type="text" class="easyui-numberbox" data-options="groupSeparator:','" disabled="disabled"/></td>
							</tr>
							<tr>
								<th>申请贷款期限(月):</th>
