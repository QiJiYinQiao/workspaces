	...
	<script type="text/javascript">
			...
			$(function() {
				...
				//加载当前用户，比如销客专员，受理任务的数据
				 $dg = $("#dg");
				 $grid=$dg.datagrid({
					url : "investOrder/investOrderAction!findAllClaimTask.action",
					width : 'auto',
					...
					columns : [ [ 
					              ...
					              {field : 'signDate',      title : '合同签署日期', width : parseInt($(this).width()*0.1), align:'center',
					            	  formatter: function(value, row, index){
 				            		  	    return value; 
					            	  }					            	  					            	  					            	  
					              },
					              {field : 'prodId',    title : '理财产品',    width : parseInt($(this).width()*0.08), align:'center' ,
					            	  formatter: function(value, row, index){
					              ...
					              {field : 'actNo',    title : '开户行帐号',   width : parseInt($(this).width()*0.05), align:'center',
					            	  formatter: function(value, row, index){
					              ...
					              {field : 'operType',       title : '操作',width : parseInt($(this).width()*0.2), align:'center', 
					            	  formatter: function(value,row,index){
		...
		function checkInvestOrderHisApprovalOpinions(index){
			...
			parent.$.modalDialog({
				...
				width : 1000,
				height : 650,
				href : "jsp/investOrder/investOrderProcessComment.jsp"});
		}		
		...
		function handleTaskDialog(index) {
			...
			$.ajax({
				type : "POST",
				url : "investOrder/investOrderAction!findTaskFormKeyByTaskId.action",
