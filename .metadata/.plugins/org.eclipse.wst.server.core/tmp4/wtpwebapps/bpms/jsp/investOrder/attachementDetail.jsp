...
<%
	...
	String basePath = request.getScheme() + "://"
		...
		+ path + "/";
	String investOrderId = request.getParameter("investOrderId");
...
<body style="background-color: #123659;">
	<div id="mws-container" class="clearfix" style="width:100%;height:100%;">
		<input type="hidden" id="investOrderId" value="<%=investOrderId%>">
...
//			rollListStr += "<ul id='' class='clearfix'><li style='width:"+$(window).width()+"px;height:"+$(window).height()*0.85+"px;margin:0;padding:0;' > <img id='imgplace' src='"+photoList[0].attURL+"' alt='' /> </li> </ul>";
			...
			photoListStr += "</ul>";
			$("#TypeList").empty();
			//下边的代码将每种类型的所有图片列表追加到该类型的图片大分类之下。
		...
		function checkAttachmentList(){
			var investOrderId = $("#investOrderId").val();			
			var userId = $("#userId").val();
			var isDetail = $("#isDetail").val();
			console.info("investOrderId = " + investOrderId + ", userId = " + userId + ", isDetail = "+isDetail);
			$.ajax({
				url : "attachment/attachmentAction!findAttachmentByOrderTypeAndOrderId.action",
				data : {"orderType":"investorOrder", "investOrderId":investOrderId,"userId":userId,"isDetail":isDetail},
				type : "post",
				async : false,
		...
		function deleteAttachmentByUrl(photoAttids){
	    	$.ajax({
		    	   url:'attachment/attachmentAction!deleteAttachmentByUrl.action',
