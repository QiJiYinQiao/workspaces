...
function createconsumablesAppGrid(){
	...
	$("#dg").datagrid({
		...
		columns:[[	{field : 'caId',        title : '编号',    width : $(this).width * 0.1, align:'center',
	                ...
	                {field : 'totalAmt',title : '合计金额(元)',width : 80,align : 'center'},
	                {field : 'allAmt',title : '总金额(元)',width : 80,align : 'center',
			        	 formatter: function(value,row,index){
	...
	$.messager.confirm('提交', '是否提交所选择的申请?', function(d) {
		if (d) {
			$.ajax( {
				...
				data : {"caId":selectedRow.consumablesApp.caId,"appNo":selectedRow.consumablesApp.appNo,},
			//	dataType:'JSON',
				success : function(rsp) {		    					    				
					if(rsp.status){
						doSearch();
