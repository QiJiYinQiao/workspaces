...
// 				createSelect();
		...
		$grid2 = $("#lookLoanOrderdgAdd").datagrid({
			...
			columns : [ [ 
			              ...
			              {field : 'branchAddr',title : '分公司地址',width : parseInt($(this).width()*0.07),align : 'center'},
			              {field : 'comUrl',title : '公司网址',width : parseInt($(this).width()*0.07),align : 'center'},
			              {field : 'appQty',title : '申请数量',width : parseInt($(this).width()*0.05),align : 'center'},
			              {field : 'price',title : '单价',width : parseInt($(this).width()*0.05),align : 'center'},
			              {field : 'subTotalAMT',title : '合计金额',width : parseInt($(this).width()*0.05),align : 'center'},
			              {field : 'unit',title : '单位',width : parseInt($(this).width()*0.03),align : 'center'},
		...
		$.ajax({
			...
			success:function(flag){
				if(flag.status){//总部
					...
					$("#applicantNo").attr("disabled",true);
				}else{
					//初始化组织机构
					$("#deptNo").combotree({
						width:171,
	   ...
	   <form id="investProductInputOrSaveForm"  method="post">
			...
			<table class="table" width="100%">
				...
				<tr>
					<th>单价:</th>
					<td>
						<input id="price" name="price" class="easyui-textbox easyui-validatebox" data-options="validType:'length[0,100]', required:true"/>
					</td>
					<th>公司网址:</th>
					<td>
