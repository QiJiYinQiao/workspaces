...
var $dialog;
var $dialog1;
var ppeListWidth = parseInt($(window).width() * 0.70);
if(ppeListWidth > 900){
	ppeListWidth = 900;
}
...
function showImage(index){
	var selectedRow = getRowData(index);
	var src = "workflowAction/showProcessImgByBusinessKey.do?processKey="+selectedRow.ppeApp.definitionKey+"&resName="+selectedRow.ppeApp.resourcesName+"&busID="+selectedRow.ppeApp.psaId;
...
function showInvestProductDetails(index){
	...
	/* 动态显示Dialog的标题	*/
	width : ppeListWidth,
	height : parseInt($(window).height() * 0.70),
	title : "查看审批意见",
