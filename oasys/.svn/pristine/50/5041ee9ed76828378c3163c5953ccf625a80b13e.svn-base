...
public class ConsumablesAppServiceImpl extends WorkFlowBaseServiceImpl implements ConsumablesAppService {
	...
	public boolean consumablesAppStartProcessInstance(ConsumablesApp consumablesApp) {
		// TODO Auto-generated method stub
				...
				consumablesApp2.setAppDate(DateUtils.getNowTime(DateUtils.DATE_FULL_STR));//将申请日期更新到提交日期
				publicDao.saveOrUpdate(consumablesApp2);
		// 获取启动实例的key
				String proDefKey = "";
	...
	public List<ConsumablesAppAttach> findConsumablesAppAttachList(
		...
		for (ConsumablesApp consumables : list) {
			...
			if(null != org.getDeptLevel() && org.getDeptLevel().equals("0")){
				...
				consumables.setDefinitionKey(Constants.ConsumablesApp_HO);
//				consumables.setTaskID(workFlowTaskService.getTaskByBusinessKey(consumables.getCaId().toString(), Constants.ConsumablesApp_HO)); //将taskID设置到实体对象中
				...
				consumables.setDefinitionKey(Constants.ConsumablesApp_BO);
//				consumables.setTaskID(workFlowTaskService.getTaskByBusinessKey(consumables.getCaId().toString(), Constants.ConsumablesApp_BO)); //将taskID设置到实体对象中
	...
	public void updateConsumablesAppStatus(Integer caId, String state) {
		...
		consumablesApp.setAppStatus(state);
		publicDao.saveOrUpdate(consumablesApp);
	}
	@Override
	...
	public List<ConsumablesAppAttach> findConsumablesListTask(
		...
		taskModel.setProcessKey(consumablesApp.getDefinitionKey());
		...
		for (WorkFlowTasksModel wl : workList) {
			for (ConsumablesApp consumables : consumablesAppList) {
				if(Integer.valueOf(wl.getBusinessID()) == consumables.getCaId()){
...
//			auditProcHis.setHandlerRole(Constants.getTaskRoleCodeMap().get(task.getTaskDefinitionKey()));
	...
	@Override
	public String saveStartProcess(ConsumablesApp consumablesApp) {
		// TODO Auto-generated method stub
		//填充taskModel对象属性
		WorkFlowTasksModel taskModel = new WorkFlowTasksModel();
		taskModel.setAppNo(consumablesApp.getAppNo());//APPNO
		taskModel.setBusinessID(consumablesApp.getCaId().toString());//业务ID
		...
		if(null != resultMap.get(Constants.STATUS_REF_ID) && StringUtils.isNotBlank(resultMap.get(Constants.STATUS_REF_ID).toString())){
			updateConsumablesAppStatus(Integer.valueOf(taskModel.getBusinessID()),resultMap.get(Constants.STATUS_REF_ID).toString());//更新流程状态
		}
	...
	public String saveTask(WorkFlowTasksModel taskModel) {
		...
		if(resultMap.get(Constants.STATUS_REF_ID)!=null){
			updateConsumablesAppStatus(Integer.valueOf(taskModel.getBusinessID()), resultMap.get(Constants.STATUS_REF_ID).toString());
		}
		return resultMap.get(Constants.RESULT_STR).toString();
	}
}
