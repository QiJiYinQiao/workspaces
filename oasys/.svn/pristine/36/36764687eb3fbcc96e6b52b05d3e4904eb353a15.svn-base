...
$(function(){
	$(window).resize(function(){
		$("#purchaseAppGrid").datagrid('resize',{
...
function toAdd(){
	var $AddDialog = $("<div></div>").dialog({    
	    title: '新增固定资产采购申请',    
	    width: $(window).width()*0.53,    
	    height: $(window).height()*0.73,    
	    closed: false,    
	    cache: false, 
	    resizable:true,
	    href: 'jsp/ad/purchasePPEApp/purchaseApp2Form.jsp',    
	    modal: true,
	    onClose : function(){
...
function sumitInvestOrder(index){
	...
	if(totalAmt==null || totalAmt==""){
		$.messager.alert("提示","请添加具体固定资产!",'info');
		return false;
	}
	...
	$.messager.confirm('确定','是否确定提交所选的数据吗?',function(flag) {
		if (flag) {
			$.ajax({
					...
					data : {"paId":row.paId,"appNo":row.appNo,"appType":row.appType,"totalAmt":totalAmt},
					success : function(msg) {
						if(msg.status){
							$datagrid.datagrid('reload');
						}
