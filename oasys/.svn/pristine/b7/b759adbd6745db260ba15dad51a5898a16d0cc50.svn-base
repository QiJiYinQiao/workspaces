...
public class UserServiceImpl implements UserService {
	...
	public List<UsersModel> findUserList(String q) {
		...
		sql.append("u.LOGIN_ACT AS '用户名',");
		sql.append("u.USER_ID AS '用户id',");
		sql.append("u.ORGANIZATION_ID AS '部门id',");
		sql.append("u.USER_NAME AS 'NAME' ");
		sql.append("FROM QQMS.t_users u LEFT JOIN QQMS.t_organization o ON u.ORGANIZATION_ID = o.ORGANIZATION_ID WHERE u.AVAILABLE = 'A'");
		if(StringUtils.isNotBlank(q)){
			sql.append(" AND u.LOGIN_ACT LIKE '%"+q+"%'");
		}
		...
		if(Collections.listIsNotEmpty(list)){
			...
			for (int i = 0; i < list.size(); i++) {
				...
				cmodel.setName(obj[4]==null?"":String.valueOf(obj[4]));
	...
	@Override
	public Long getUserByOrgAndRole(String orgID,String roleCode){
		if(StringUtils.isBlank(orgID) || StringUtils.isBlank(roleCode)){
			return Long.parseLong("0");
		}
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT count(tu.user_id) FROM ");
		sql.append("qqms.t_users tu LEFT JOIN qqms.t_user_and_role ur ON tu.user_id = ur.user_id ");
		sql.append("LEFT JOIN qqms.t_role tr ON ur.role_id = tr.ROLE_ID ");
		sql.append("WHERE tu.ORGANIZATION_ID = "+orgID+" ");
		sql.append("AND tr.ROLE_CODE = '"+roleCode+"' ");
		return publicDao.findTotalCount(sql.toString());
	}
	
	@Override
	public List<String> getUserIdsByOrgAndRoleOrg(String orgID,String roleCode){
		if(StringUtils.isBlank(orgID) || StringUtils.isBlank(roleCode)){
			return null;
		}
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT tu.user_id FROM ");
		sql.append("qqms.t_users tu LEFT JOIN qqms.t_user_and_role ur ON tu.user_id = ur.user_id ");
		sql.append("LEFT JOIN qqms.t_role tr ON ur.role_id = tr.ROLE_ID ");
		sql.append("WHERE tr.ROLE_CODE = '"+roleCode+"' ");
		sql.append("AND ur.joint_organ_city LIKE '%@"+orgID+"@%';");
		List<Object> list = publicDao.findBySQL(sql.toString());
		List<String> strList = new ArrayList<String>();
		for(Object item : list) {
			strList.add(item == null?"":item.toString());
		}
		return strList;
	}
	...
	//加载用户名及机构
		...
		public List<UsersModel> getUserInfo() {
			List<Object[]> list=publicDaoUsersModel.findBySQL("SELECT u.USER_NAME,o.FULL_NAME,u.user_id FROM qqms.t_users u LEFT JOIN qqms.t_organization o ON u.ORGANIZATION_ID = o.ORGANIZATION_ID");
	...
	@Override
	public List<Users> findUsersByOrgId(Integer orgId) {
		StringBuffer hql = new StringBuffer();
		hql.append("select * from Users u where u.organizeId = '"+orgId+"'");
