...
function createconsumablesAppGrid(){
	...
	$("#dg").datagrid({
		...
		columns:[[	{field : 'caId',        title : '编号',    width : $(this).width * 0.1, align:'center',
		            ...
		            {field : 'myId',title : '所属业务端',width : 100,align : 'center',
							        	 formatter: function(value,row,index){
							        		 if(row.consumablesApp.myId=="CF"){
							        			 return "财富端";
							        		 }else if(row.consumablesApp.myId=="JK"){
							        			 return "借款端";
							        		 }else{
	                ...
	                {field : 'totalAmtStr',title : '合计金额(元)',width : 80,align : 'center'},
	                {field : 'allAmtStr',title : '总金额(元)',width : 80,align : 'center',
			        	 formatter: function(value,row,index){
	                ...
	                {field : 'caozuo',title : '操作',width :180,align : 'center',formatter:function(value,row,index){
	                	if(row.consumablesApp.procStatus == "1"){
	             	    	return "<a href='javascript:void(0);' onclick='toSaveOrUpdatePpeScrap("+row.consumablesApp.caId+");'>编辑</a>&nbsp;&nbsp;"+"<a href='javascript:void(0);' onclick='deleteConsumablesApp("+ index + ");'>删除</a>&nbsp;&nbsp;"+"<a href='javascript:void(0);' onclick='sumitLoanOrder("+index+");'>提交申请</a>";
	             	    }else if(row.consumablesApp.procStatus == "2"){
...
function toSaveOrUpdatePpeScrap(caId){
	$("#addWindow").dialog({
		...
		onLoad:function(){
			/* if(""!=caId && null!=caId){
				$("#rowOne").attr("class","dis");
				$("#rowTwo").attr("class","dis");
				$("#rowThirth").attr("class","");
...
function sumitLoanOrder(index){
	var selectedRow = getRowData(index);
	if(null==selectedRow.consumablesApp.allAmt || ""==selectedRow.consumablesApp.allAmt){
	...
	$.messager.confirm('提交', '是否提交所选择的申请?', function(d) {
		if (d) {
			$.ajax( {
				type : "POST",
				url : 'consumablesApp/saveStartProcess.do',
				data : {"caId":selectedRow.consumablesApp.caId,"appNo":selectedRow.consumablesApp.appNo,"allAmt":selectedRow.consumablesApp.allAmt},
			//	dataType:'JSON',
				success : function(rsp) {		    					    				
					if(rsp.status){
						doSearch();
...
<div class="well well-small" style="margin-left: 5px;margin-top: 5px">
	<form id="searchForm" action="consumablesApp/findConsumablesAppList" style="min-width: 1200px;">
	  <table>
	    <tr>
	      <td>申请状态：&nbsp;&nbsp;</td>
	...
	<table cellpadding="0" cellspacing="0">
		<tr>
			<td style="padding-left: 2px">
				<a id="id4ExportReports" href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="toSaveOrUpdatePpeScrap('');">添加</a>
			</td>
		</tr>
		</table>
	</div>
