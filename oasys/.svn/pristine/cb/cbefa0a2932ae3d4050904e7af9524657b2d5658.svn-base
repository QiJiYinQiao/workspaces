...
public class EmpForgetPluginAppProcessStopListener implements ExecutionListener {

	@Override
	public void notify(DelegateExecution execution) throws Exception {
		WebApplicationContext ctx = ContextLoader.getCurrentWebApplicationContext();
		EmpForgetPluginAppService empForgetPluginAppService = (EmpForgetPluginAppService) ctx.getBean("empForgetPluginAppService");
		...
		if(execution.getVariable(Constants.CURRENT_BUSINESS_ID) != null){
			...
			if(task.getTaskDefinitionKey().startsWith(Constants.APPLY_FOR_ADJUSTMENT) && "0".equals(execution.getVariable("result"))){
				empForgetPluginAppService.updateEmpForgetPluginAppProcessStatus(id, "5");
			}else{
				empForgetPluginAppService.updateEmpForgetPluginAppProcessStatus(id, "3");
			}
		}
	}
