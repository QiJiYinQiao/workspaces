	...
	$(function() {
		...
		$grid =$("#travelExpensesAppgrid").datagrid({
					...
					columns : [ [
							...
							{field : 'insuranceBgDtime',title : '保险开始时间(保险)',width : parseInt($(this).width()*0.12),align : 'center'},
							{field : 'insuranceEdDtime',title : '保险结束时间(保险)',width : parseInt($(this).width()*0.12),align : 'center'},
		 ...
		 $saveOrUpdbanliDialog = $("#saveDialog").dialog({
				...
				onLoad:function(){
					if(index!=null){
						...
						f.form("load",$row);
						$("#insuranceEdDtime").datebox('setValue',$row.insuranceEdDtime);
						$("#feeTypeId").val($row.feeType);
...
<body>
		...
		<!-- 意见 -->
		<div id="optionsDialog" class="easyui-dialog" title="历史审批意见" style="width:900px;height:500px;" closed="true">
