...
// 		
		...
		aDisable();
		//渲染申请人列表appPeoNo
		badgePeopleGrid($("#appNofo").val()==""?"null":$("#appNofo").val());
	...
	function toSaveBaseInfo(typt){
		...
		if($("#branchAddr").val()=="" && $("#appPeoNo").val()==''){
			$.messager.alert("提示","请填写地址!","info");
			return false;
		}
		if($("#appQty").val()=="" && $("#appPeoNo").val()==''){
    ...
    function badgePeopleGrid(appNo){
 	 ...
 	 function toEditAtta(){
 		var selected = $('#appUserView').datagrid('getSelected');
	    	...
	    	/* $.ajax({
				...
				success:function(res){
					if(res!=null){
						...
						$.messager.alert("提示", '名片申请修改成功!',"info")
