...
public class PurchaseAppController extends BaseController{
    ...
    public String searchPurchaseAppList(HttpServletResponse httpServletResponse,Integer page,Integer rows,PurchaseAppModel purchaseAppModel){
    	...
    	gridModel.setTotal(purchaseAppService.countPurchaseAppList(purchaseAppModel));
    ...
    public String startWorkflow(HttpServletResponse httpServletResponse,PurchaseApp purchaseApp){
		...
		String resultStr = "";
		try {
			resultStr = purchaseAppService.startWorkflow(purchaseApp);
		} catch (Exception e) {
    ...
    public String findAllPurchaseAppTaskList(HttpServletResponse httpServletResponse,Integer page,Integer rows,WorkFlowTasksModel workFlowTasksModel){
        GridModel gridModel = new GridModel();
        gridModel.setRows(purchaseAppService.findAllPurchaseAppTaskList(page,rows,workFlowTasksModel));
        gridModel.setTotal(purchaseAppService.countAllPurchaseAppTaskList(workFlowTasksModel));
        OutputJson2(httpServletResponse, gridModel);
    	return null;
    }
    /**
    ...
    public String submitTask(HttpServletResponse httpServletResponse,@ModelAttribute("taskModel") WorkFlowTasksModel taskModel,  BindingResult bindingResult){
		...
		try {
			//执行受理申请
			resultStr = purchaseAppService.saveSubmitTask(taskModel);
		} catch (Exception e) {
			saveSuccess = false;
		}
    ...
    public String submitTaskBatch(HttpServletResponse httpServletResponse,@ModelAttribute("taskModel") WorkFlowTasksModel taskModel,  BindingResult bindingResult){
		...
		try {
			//执行受理申请
			purchaseAppService.saveSubmitTaskBatch(taskModel);
		} catch (Exception e) {
			saveSuccess = false;
		}
