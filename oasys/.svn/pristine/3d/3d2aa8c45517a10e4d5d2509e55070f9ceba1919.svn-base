...
<script type="text/javascript">
	...
	function RenderName(organizeId){
		...
		$("#userName").combobox({
			...
			multiple:true,
			separator:",", 
			url:"BadgeApp/findUserListByDept.do?organizeId="+organizeId,
			valueField:'code',
		 	textFiled:'text',
	...
	function saveOrDepBadge(typt){
		$.ajax({
			...
			success:function(res){
				 if(res!=null){
					if(typt==1){
						$("#saveOrUpdateEmpSalDialog").dialog('destroy');
    ...
    $("#upploadAttachment").click(function(){
    	if($.trim($("#appPeoNo").val())!=''){
    		fileUploadsDlg($("#appPeoNo").val(),$("#registrantNo").val());
    	}
	});
	//查看附件
	$("#checkAttachment").click(function(){
		if($.trim($("#appPeoNo").val())!=''){
			checkAttachementDetail($("#appPeoNo").val(),$("#registrantNo").val());
    	}
	});
...
<div id="tt" >
	<div title="申请人信息" >
	  <div class="well well-small" style="margin:5px;width:795px">
	         ...
	         <input id="registrantNo" name="registrantNo" type="hidden"/><!-- 登记人 -->
	         ...
	         <table class="table">
				<tr id="user">
				 ...
				 <tr>
					<th>备注:</th>
					<td colspan="3">
					  <textarea id="remark" name="remark" class="easyui-textbox easyui-validatebox" style="width: 700px; height: 80px;resize:none;" data-options="validType:'length[0,200]'"></textarea>
					</td>
				</tr>
				<tr>
