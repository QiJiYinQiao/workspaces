...
function submitTask(result,object){
	if(result=="renliziyuanjinglibohui"){
	// 验证备注信息是否已经填写
		if($("#comment").val()==""){
	...
	$.messager.confirm('提示', '点击按钮之后将进入下一个任务活动环节,此任务将对您不可见!', function(r){
		if (r){
			var data = {
				...
				"appNo":$row.appNo,
				"taskId":$row.taskId,
				"handleResult":result=='renliziyuanzongjiantongguo'?1:result=='renliziyuanzongjianbohui'?0:2
			}
			$.ajax({
				type:"POST",
		 ...
		 <table class="table" cellpadding="5px;">
			...
			<tr>
				<td colspan="8">
					...
					<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('renliziyuanzongjiantongguo',this);">申请通过</a>
					<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('renliziyuanzongjianbohui',this);">申请驳回</a>
					<a href="javascript:void(0);" class="easyui-linkbutton" onclick="submitTask('renliziyuanzongjianjujue',this);">申请拒绝</a>
				</td>
			</tr>
			<tr>
				<td colspan="8">
