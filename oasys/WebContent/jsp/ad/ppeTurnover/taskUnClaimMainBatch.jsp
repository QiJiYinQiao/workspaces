			    ...
			    %>
				 $grid=$("#dg").datagrid({
	              ...
	              onLoadSuccess:function(data){
	            	  var paramObj = {rowStr:'appDeptName,appNo',//按照哪列进行分组
	      					rowGroup:'appDeptName,heji:ck,appNo,name,ppeTotalAmt,appDate,caozuo',//对应rowStr的需要合并单元格的字段
	      					hejiColumn:'ppeTotalAmt',//需要计算的合计值的所在列
			...
			function signTask(){
				...
				for(var i=0;i<rows.length;i++){
					if($.inArray(rows[i].ppeTurnoverApp.ptaId,businessID)==-1){
						...
						formKey.push(rows[i].ppeTurnoverApp.formKey);
