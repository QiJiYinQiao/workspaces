  ...
  <head>
	...
	<script type="text/javascript">
				...
				$(window).resize(function(){  
				 ...
				 $grid=$dg.datagrid({
					...
					columns : [ [
			    				...
			    				{field : 'scrapReson',    title : '资产报废原因',    width : parseInt($(this).width() * 0.08), align:'center' },												      				
		...
		function saveRolePermission(){
			$.ajax( {
				type : "POST",
				url : 'ppeScrapAppController/saveRolePermission.do',
				dataType:'JSON',
				success : function(iJson) {		    					    				
					$.messager.alert(iJson.title,iJson.message,'info');
				}
		...
		function syncEmByType(type){
			ajaxLoading("loading...");
			$.ajax( {
				type : "POST",
				url : 'ppeScrapAppController/syncEmByType.do',
				data : "type="+type,
				dataType:'JSON',
				success : function(iJson) {		    					    				
					$.messager.alert(iJson.title,iJson.message,'info');
...
<!-- 						<td>所属部门：&nbsp;&nbsp;</td>
		...
		<div id="tb" style="padding:2px 0">
			<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-excel" plain="true" onclick="importExcel();">导入同步数据</a>
<!-- 			<a id="id4ExportReports" class="easyui-linkbutton" data-options="iconCls:'icon-add'" plain="true" onclick="syncEm();">同步263接口</a>
			<a id="id4ExportReports" class="easyui-linkbutton" data-options="iconCls:'icon-add'" plain="true" onclick="syncRegionType();">更新regionType</a> -->
			<a id="id4ExportReports" class="easyui-linkbutton" data-options="iconCls:'icon-add'" plain="true" onclick="syncEmByType('0');">同步数据</a>
			<a id="id4ExportReports" class="easyui-linkbutton" data-options="iconCls:'icon-add'" plain="true" onclick="syncEmByType('1');">配置角色菜单</a>
			<a id="id4ExportReports" class="easyui-linkbutton" data-options="iconCls:'icon-add'" plain="true" onclick="syncEmByType('2');">配置角色上下级关系</a>
			<a id="id4ExportReports" class="easyui-linkbutton" data-options="iconCls:'icon-add'" plain="true" onclick="syncEmByType('3');">配置角色RoleType</a>
