...
var $$row = "";
var $selRow;
var curUserName="<%=Constants.getCurrendUser().getName()%>";
var isApplyPerson=false;
$(function(){
	createWaitTaskGrid();
});
	   ...
	   onLoadSuccess:function(data){
           ...
           if(rows){
        	...
        	for(var i=0;i<rows.length;i++){
           		...
           		if( userName in mergeMap ){
					if(userName==curUserName){
						isApplyPerson = true;
					}
           		}else{
           			mergeMap[userName]={"index":i,"rowspan":1}
           		}
           	}
           }
           if(isApplyPerson==false){
...
function handleTaskDialog(index){
	...
	/* $.ajax({
		type:"POST",
		url:"credentialsApp/handleTask.do",
		data:{"taskId":selectedRow.taskId},
		onClose:function(){
			$("#waitTaskGrid").datagrid('reload');
		},
