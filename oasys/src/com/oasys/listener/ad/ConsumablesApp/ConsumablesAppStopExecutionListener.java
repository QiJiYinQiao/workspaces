	...
	public void notify(DelegateExecution execution) throws Exception {
		if(execution.getVariable(Constants.CURRENT_BUSINESS_ID) != null){
			...
			if(Constants.TASK_REFUSE_RESULT.equals(execution.getVariable("result"))){
				consumablesAppService.updateConsumablesAppProceStatus(id, Constants.PROC_REFUSE);
			// 修改订单的状态 判断状态是否为申请撤销 如果的话 将流程状态更新为已撤销
				...
				consumablesAppService.updateConsumablesAppProceStatus(id,Constants.PROC_REVOKE);
			}else{
				consumablesAppService.updateConsumablesAppProceStatus(id,Constants.PROC_COMPLETED);
			}
		}
	}
}
