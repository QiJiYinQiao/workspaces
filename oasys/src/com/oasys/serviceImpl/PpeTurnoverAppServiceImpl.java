...
//			sql.append("t1.APP_DATE,");
			sql.append("u.LOGIN_ACT  as '申请人姓名', ");
			sql.append("oo.FULL_NAME as '接收机构名称',");
			sql.append("uu.LOGIN_ACT as '接收人姓名', ");
			sql.append("t1.APP_DATE, ");
...
//				ppeTurnoverApp.setInvestorStatus("I");
	...
	public boolean startWorkflow(PpeTurnoverApp ppeTurnoverApp) {
		...
		if (Collections.listIsNotEmpty(ppeTurnoverAttachList)) {
			...
			ppeTurnoverApp2.setAppDate(new Date());//将申请日期更新到提交日期
	...
	public List<PpeTurnoverAttach> findAllPpeTurnoverAppTaskList(Integer page,
			Integer rows,PpeTurnoverApp ppeTurnover) {
		//登录角色所要办理的的任务集合
		WorkFlowTasksModel taskModel = new WorkFlowTasksModel();
		taskModel.setProcessKey(ppeTurnover.getDefinitionKey());
				List<WorkFlowTasksModel> taskModelList = workFlowTaskService.findAcceptTaskByGroup(taskModel);
				...
				if (Collections.listIsNotEmpty(taskModelList)) {
					...
					for (WorkFlowTasksModel wl : taskModelList) {
						for (PpeTurnoverApp ppeTurnoverApp : pamList) {
							...
							if (Integer.valueOf(wl.getBusinessID()) == ppeTurnoverApp.getPtaId()) {
...
/*	@Override
	...
	try {
		...
		Integer userId = Constants.getCurrendUser().getUserId();
		updatePpeTurnoverappStatus(ppeTurnoverApp.getPtaId(),result);// 修改订单的状态
	...
	@Override
	public void updatePpeTurnoverappStatus(Integer ptaId, String state) {
		//跟新订单状态
		PpeTurnoverApp ppeTurnoverApp = publicDao.get(PpeTurnoverApp.class, ptaId);
		ppeTurnoverApp.setAppStatus(state);
		publicDao.saveOrUpdate(ppeTurnoverApp);
	...
	public String saveTask(WorkFlowTasksModel taskModel) {
		...
		if(resultMap.get(Constants.STATUS_REF_ID)!=null){
			updatePpeTurnoverappStatus(Integer.valueOf(taskModel.getBusinessID()), resultMap.get(Constants.STATUS_REF_ID).toString());
		}
		return resultMap.get(Constants.RESULT_STR).toString();
	}
	@Override
	public String saveStartProcess(PpeTurnoverApp ppeTurnoverApp) {
		// TODO Auto-generated method stub
		//填充taskModel对象属性
		WorkFlowTasksModel taskModel = new WorkFlowTasksModel();
		taskModel.setAppNo(ppeTurnoverApp.getAppNo());//APPNO
		taskModel.setBusinessID(ppeTurnoverApp.getPtaId().toString());//业务ID
...
//		taskModel.setApplyStr(Constants.APPLY_FOR_ADJUSTMENT);//调整申请节点的标识
		...
		if(null != resultMap.get(Constants.STATUS_REF_ID) && StringUtils.isNotBlank(resultMap.get(Constants.STATUS_REF_ID).toString())){
			updatePpeTurnoverappStatus(Integer.valueOf(taskModel.getBusinessID()),resultMap.get(Constants.STATUS_REF_ID).toString());//更新流程状态
		}
		return resultMap.get(Constants.RESULT_STR).toString();
	}
	private String getProcDefKey() {
		...
		// 获取启动实例的key
				String proDefKey = "";
				Organization org = organizationService.findOrganizationByUserId(Constants.getCurrendUser().getUserId());
				//0为总部 1为分部
	...
	@Override
	public PpeStockInfo findPpeStock(PpeStockInfo ppeStockInfo) {
		// TODO Auto-generated method stub
		String roleCode = userService.findRoleListByUserId(Constants.getCurrendUser().getUserId()).get(0).getRoleCode();
		if (roleCode.equals("XingZhengZhuanYuan")) {
			ppeStockInfo.setPpeStatus("1");
		} else {
			ppeStockInfo.setPpeStatus("2");
			ppeStockInfo.setCustodian(Constants.getCurrendUser().getUserId());
		}
		PpeStockInfo ppeStock = ppeStockInfoService.getPpeStockInfo(ppeStockInfo);
		return ppeStock==null?null:ppeStock;
	}
	@Override
		...
		if(Collections.listIsNotEmpty(resultMapList)){
			for (Map<String, Object> map : resultMapList) {
				if(map.containsKey(Constants.STATUS_REF_ID) && map.containsKey(Constants.BUSINESS_ID)){
					updatePpeTurnoverappStatus(Integer.valueOf(map.get(Constants.BUSINESS_ID).toString()),map.get(Constants.STATUS_REF_ID).toString());// 更新流程状态
