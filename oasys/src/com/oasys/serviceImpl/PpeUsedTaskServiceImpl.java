...
public class PpeUsedTaskServiceImpl  extends WorkFlowBaseServiceImpl  implements PpeUsedTaskService
{
	...
	public String addStartProcessInstance(Integer psaId) {
		try {
			...
			//字表中的id集合
			List<String> assigneeList = ppeUsedConfirmService.findPpeUsedAttPsaIdList(ppeUsedConfirmApp.getAppNo());
			...
			if(Collections.listIsNotEmpty(list)){
				...
				for (int i = 0; i < list.size(); i++) {
					...
					for (WorkFlowTasksModel wl : workList) {
						String taskDefinitionKey = this.taskService.createTaskQuery().taskId(wl.getTaskID()).singleResult().getTaskDefinitionKey();
						if(taskDefinitionKey.equals("ApplyForAdjustment2")){
							String variable = this.taskService.getVariable(wl.getTaskID(), "${assignee}").toString();
								if(variable.equals(String.valueOf(app.getPuaPsaId()))){
									app.setTaskModel(wl);
								...
								if( Integer.valueOf(wl.getBusinessID()).intValue() == app.getPsaId().intValue() ){
									app.setTaskModel(wl);
									app.setTaskID(wl.getTaskID());
									app.setAssistant(wl.getAssistant());
									app.setFormKey(wl.getFormKey());
	...
	public Long findPpeUsedTaskTotal(PpeUsedConfirmApp ppeUsedConfirmApp) {
		...
		try {
			...
			sql.append("SELECT 	puc.PSA_ID '申请id', pua.PSA_ID '详情表id' ");
			...
			List list = ppeUsedDao.findBySQL(sql.toString());
			...
			if(Collections.listIsNotEmpty(list)){
				...
				for (int i = 0; i < list.size(); i++) {
					...
					PpeUsedConfirmApp app=(PpeUsedConfirmApp) confirmApp.clone();
					app.setPsaId(obj[0]==null?0:(Integer)obj[0]);//申请id
					app.setPuaPsaId(obj[1]==null?0:(Integer)obj[1]);//附加表id
